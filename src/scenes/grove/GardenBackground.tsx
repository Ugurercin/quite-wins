...
// Real fluffy cloud
// ──────────────────────────────────────────────
function Cloud({ x, y, s, opacity }: { x: any; y: number; s: number; opacity: number }) {
  ...
  return (
    <>
      <RoundedRect x={baseX} y={18 * s + y} width={70 * s} height={20 * s} r={10 * s} color="#FFFFFF" opacity={opacity} />
      ...
      <RoundedRect x={baseX} y={28 * s + y} width={70 * s} height={10 * s} r={5 * s} color="#D0D8E8" opacity={opacity * 0.35} />
    </>
  )
}
...
// Butterfly
// ──────────────────────────────────────────────
function Butterfly({ t, w, h, colorA, colorB, seed, groundY }: {
  t: any; w: number; h: number; colorA: string; colorB: string; seed: number; groundY: number
}) {
  ...
  return (
    <>
      ...
      <Circle cx={useDerivedValue(() => bodyX.value - 4.5)} cy={useDerivedValue(() => bodyY.value + 3)} r={lwR} color={colorB} opacity={0.85} />
      <Circle cx={useDerivedValue(() => bodyX.value + 4.5)} cy={useDerivedValue(() => bodyY.value + 3)} r={lwR} color={colorB} opacity={0.85} />
  ...
  return (
    <>
      ...
      {geo.bgGrass.paths.map((p, i) => (
        <Path key={`bgl${i}`} path={p} color={grassColors[(geo.bgGrass.colorIndices[i] + 2) % grassColors.length]} style="fill" opacity={0.46} />
      ))}
      ...
      <Rect x={0} y={groundY} width={w} height={h - groundY}>
        <LinearGradient start={vec(0, groundY)} end={vec(0, h)} colors={[GRASS_GROUND, GRASS_DEEP]} />
      </Rect>
      ...
      {geo.midGrass.paths.map((p, i) => (
        <Path key={`mgl${i}`} path={p} color={grassColors[(geo.midGrass.colorIndices[i] + 1) % grassColors.length]} style="fill" opacity={0.72} />
      ))}
