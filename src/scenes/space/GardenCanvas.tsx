...
import { resolveSpacePosition } from './spacePositions'
...
import { PlantType, PLANT_TYPES } from './plants/plantTypes'

const toSpaceType = (t: string | undefined): PlantType =>
  PLANT_TYPES.includes(t as PlantType) ? (t as PlantType) : 'terrestrial'
...
const GardenCanvas = ({ width, height, colors, theme, plants, wins, activeSceneId, onPlantTap }: CanvasProps) => {
  ...
  const getAccentColor = (plant: Plant): string => {
    ...
    const win = wins.find((w: Win) => w.id === lastWinId)
    ...
    <View style={{ width, height }}>
      <Canvas style={{ width, height }}>
        ...
        {visiblePlants.map(plant => {
          ...
          const { x, y } = resolveSpacePosition(plant.slotIndex, width, height)
          return (
            <PlantNode
              key={plant.id}
              x={x}
              ...
              plantType={toSpaceType(plant.plantType)}
              isElder={plant.isElder}
            />
          )
        })}

        {departingPlants.filter(p => !p.isElder || (p.sceneId ?? 'grove') === activeSceneId).map(plant => {
          const { x, y } = resolveSpacePosition(plant.slotIndex, width, height)
          return (
            <PlantNode
              key={`dep-${plant.id}`}
      ...
      {visiblePlants.map(plant => {
        ...
        return (
          <TouchableOpacity
            ...
            activeOpacity={0.0}
            accessibilityLabel="View planet details"
          />
        )
      })}
    </View>
